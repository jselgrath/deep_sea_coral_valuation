...
# --------------------------------------------------
fxn_commsector_catch1 <- function(dta){
  dta%>% 
  filter(year >= 2010) %>%
  drop_na(Value) %>%
...
cc_bl<-fxn_commsector_catch1(d1_bl)
cc_pr<-fxn_commsector_catch1(d1_pr)
cc_ha12<-fxn_commsector_catch1(d1_ha12)
cc_ha2<-fxn_commsector_catch1(d1_ha2)
cc_all<-fxn_commsector_catch1(d1_all)
...
fxn_econ_contr<-function(dta){
  dta %>%
    ...
    select(-Vessel_output,-Vessel_income,-Vessel_employment,-Processor_output,-Processor_income,-Processor_employment)%>%
    glimpse()
}
...
fxn_econcontr_summary_CA <-function(dta){
  dta%>%
    group_by(year,assoc) %>%
    dplyr::summarise(Revenue = sum(revenue, na.rm = T),
                     Vessel_output = sum(Vessel_output2, na.rm = T),
                     Vessel_income = sum(Vessel_income2, na.rm = T),
                     Vessel_employment = sum(Vessel_employment2, na.rm = T),
                     Processor_output = sum(Processor_output2, na.rm = T),
                     Processor_income = sum(Processor_income2, na.rm = T),
                     Processor_employment = sum(Processor_employment2, na.rm = T),
                     TotOut = sum(TotOut, na.rm = T),
                     TotInc = sum(TotInc, na.rm = T),
                     TotEmp = sum(TotEmp, na.rm = T))%>%
    glimpse()
...
fxn_econcontr_CA_report <- function(dta){
  dta%>%
  select(year, assoc, Revenue,TotOut,TotInc,TotEmp)%>%
  glimpse()
}
